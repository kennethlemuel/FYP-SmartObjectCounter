...
def main():
    ...
    ap.add_argument("--save_dir", type = str, default = "outputs/rgbtcc_t")
    args = ap.parse_args()
    ...
    train_ds = RGBTCC_TDataset(args.data_root, "train", (args.img_h, args.img_w), args.sigma)
    val_ds   = RGBTCC_TDataset(args.data_root, "val",   (args.img_h, args.img_w), args.sigma)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        torch.save({"epoch": ep, "model": model.state_dict()},
                   os.path.join(args.save_dir, "last_t.pth"))
        if mae < best_mae:
            best_mae = mae
            torch.save({"epoch": ep, "model": model.state_dict()},
                       os.path.join(args.save_dir, "best_t.pth"))
            print(f"-> saved best_t.pth (MAE {best_mae:.2f})")
